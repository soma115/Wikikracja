...
def mark_as_read(object_id: int, user) -> None:
    ...
    ReadStatus.objects.get_or_create(user=user, content_type=ReadStatus.ContentType.TASK, object_id=object_id)


def mark_as_unread(object_id: int, user) -> None:
    from home.models import ReadStatus

    ReadStatus.objects.filter(user=user, content_type=ReadStatus.ContentType.TASK, object_id=object_id).delete()
