...
- [x] **D.4.4 Logika licznika znaków ×3** — wyekstrahowano `window.applyCounterState(counterEl, remaining)` do `common/js/char-counter.js` (ładowany w `base.html`, dostępny dla modułów ES i klasycznych skryptów). Wszystkie trzy miejsca (`chat/handlers.js`, `richtext-core.js::updateCounter`, `textarea-counter.js`) wołają teraz wspólny helper; `handlers.js` zachowuje dodatkowo `tw-compose-box--error` i `sendBtn.disabled` przy `<=0`.
- [x] **D.4.5 Wspólny partial pustego stanu** — utworzono `home/templates/home/includes/empty_state.html` z parametrami `icon`/`title`/`text`/`cta_*`/`extra_class`/`hidden`. Kanoniczne klasy: `tw-empty-state` + `tw-empty-state-icon`/`tw-empty-state-title`. Zmigrowano: `tasks/_empty_state.html` (deleguje do wspólnego), `ankiety/survey_list.html`, `glosowania/list.html`, `events/event_list.html` (z `extra_class='tw-events-empty'` jako hook JS i `hidden=occurrences`), `activity.html` (inline markup → partial). Usunięto reguły `.tw-proposals-empty`/`.tw-events-empty*` ze źródła CSS i safelisty (`tw-proposals-empty` → `tw-empty-state` + `tw-empty-state-title`). `tw-empty-chat-message` zostaje — to drobny paragraf renderowany z JS, nie server-side partial.
- [x] **D.4.6 `window.showToast` zdefiniowane w module czatu** — przeniesiono do `common/js/toast.js` (ładowane w `base.html` przed `app.js`). Klasy ujednolicone: `tw-toast-chat`/`tw-toast-chat--visible` → `tw-toast`/`tw-toast--visible`, element `#tw-toast` (zamiast `#chat-toast`). `handlers.js` zachowuje lokalny alias `showToast` wołający `window.showToast`. Safelist zaktualizowany.
- [x] **D.4.7 `data-arg-form-toggle` → wspólny `tw-collapse`** — `szczegoly.html`: `data-arg-form-toggle` → `data-tw-toggle="collapse" data-tw-target="#form-*"`, formy dostały `tw-collapse` + `data-tw-autofocus`. `tw-collapse.js` dostał opcjonalny `data-tw-autofocus` (fokus pierwszego pola po `show()` — zachowuje dawny focus textarea). Usunięto `window.toggleArgForm` + delegację z `app.js` i `display:none`/`.tw-visible` z `.tw-arg-inline-form`.

### D.5 Dalsza unifikacja (helpery JS, inline `on*`, chip-filter, i18n)

- [x] **D.5.1 Wspólne utilsy w `common/js/dom-utils.js`** — jedno źródło: `window.escapeHtml` (re-export z `utility.js`; konsumuje `tasks.js`, `tw-popover.js`), `window.getCSRFToken` (konsumują `utility.js`, `chat-core.js`, `app.js`; usunięto `window.LINK_TITLES_CSRF_TOKEN` z `base.html` i `window.CSRF_TOKEN` z `activity.html`/`home.html`), `window.apiFetch(url, {method, body, fetchOptions})` (JSON/FormData/URLSearchParams; konsumują `app.js` ×3, `handlers.js`, `push-notifications.js` ×2, `utility.js::sendNotificationAck`, `activity.html`), `window.debounce` (konsumuje `citizens_list.js`).
- [x] **D.5.2 Inline `on*` → delegaty `data-tw-*`** — w `dom-utils.js` jeden delegat obsługuje: `data-tw-stop-propagation` (32×, zamiast `onclick="event.stopPropagation()"`), `data-tw-back` (+`data-tw-back-skip-referrer` dla warunkowego `history.go(-2)`), `data-tw-remove="selector"` (zamykanie `tw-toast-msg` w `base.html`), `data-tw-submit-once` (`post_form.html`), `data-tw-confirm` (formularz usuwania załącznika). Aktualizacja `UI_STANDARDS.html`.
- [x] **D.5.3 Wspólny partial chip-filter** — `home/includes/chip_filter.html` (`all_label`, `all_icon`, `all_selected`, `items`, `selected`, `input_name`); zmigrowano `activity.html` i `search.html`. `id="sp-select-all"` to kontrakt `initChipFilters` w `app.js` — udokumentowany w partialu.
- [x] **D.5.4 Stringi PL w JS → `_()`** — `handlers.js` (trim-toast ×2, rename room: pusta nazwa/błąd/sukces/brak połączenia) i `chat-embedded.js` (empty state, brak dostępu) przeszły na `_()`; nowe klucze dodane do `chat/i18n.py::get_translations`.


### D.6 Modal header + citizens data-detail-url

- [x] **D.6.1 Wspólny nagłówek modala** — nowy partial `home/includes/modal_header.html` (`title`, `title_icon`, `title_class`, `title_id`; w środku `tw-modal-header` + `tw-modal-title` + `tw-btn-close` z `data-tw-dismiss="modal"`). Zmigrowano 7 nagłówków: `chat.html` ×3, `categories/_category_modal.html`, `obywatele/my_profile.html`, `tasks/task_detail.html`, `tasks/task_list.html`. `tw/layout/modal.html` zostaje — to crispy-form layout, inny system.
- [x] **D.6.2 `citizens_list.js` → `data-detail-url`** — usunięto własne `click`/`data-href` handlery; wiersze `.tw-user-row` i karty `.tw-citizen-card` w `start.html`/`poczekalnia.html` mają teraz `data-detail-url` + `role="link" tabindex="0"` (kontrakt `app.js`). Keydown-handler `data-detail-url` dostał guard `closest('a, button, input, select, textarea')`, żeby Enter na wewnętrznym linku nie nawigował karty.


### D.7 Druga runda unifikacji

- [x] **D.7.1 Skopiowany modal kategorii** — `tasks/task_list.html` zawierał identyczną kopię `categories/_category_modal.html` (te same ID `catMgr*`); zamieniono na `{% include 'categories/_category_modal.html' %}`. Wspólny partial dostał `title_class='tw-text-base tw-font-semibold'` + `fa-fw` na ikonie (dotychczasowy wygląd wersji z tasks).
- [x] **D.7.2 Jeden kontrakt klikalnych wierszy/kart** — `tr[data-href]` w `_citizen_aktywnosc/_czaty/_zadania/_zalozono` zmigrowano na `data-detail-url` + `role="link" tabindex="0"`; usunięto drugi, równoległy handler `.tw-table-hover tbody tr[data-href]` z `app.js` (całość obsługuje teraz delegat `data-detail-url`).
- [x] **D.7.3 `window.readJsonScript(id, fallback)`** — helper w `dom-utils.js` zastępujący boilerplate `JSON.parse(document.getElementById(id).textContent)` w `chat.html` (4×), `_embedded_chat.html` (4×), `bookkeeping/transaction_form.html` (1×).
- [x] **D.7.4 Wspólny init managera kategorii** — nowy partial `categories/_category_manager_init.html` (parametry: `list_url`, `reorder_url`, `with_items`, `items_suffix`, `confirm_del`) zastąpił dwie niemal identyczne sekcje `initCategoryManager` w `board/board.html` i `tasks/task_list.html`.
- [x] **D.7.5 Strony potwierdzenia usunięcia** — `bookkeeping/transaction_confirm_delete.html` przepisany na `extends bookkeeping/confirm_delete.html` (widok dostał `cancel_url` w `get_context_data`); para przycisków Delete/Cancel wyekstrahowana do `home/includes/confirm_delete_buttons.html` (`cancel_url`, `confirm_label`, `cancel_label`, `btn_sm`, `cancel_first`) i zmigrowana w `board/post_confirm_delete`, `board/attachment_confirm_delete`, `events/event_confirm_delete` (cancel_first), `glosowania/delete_argument` (btn_sm).
