...
class BuildChatMessagePayloadTest(TestCase):
    ...
    def test_payload_user_id_is_none_when_user_missing(self):
        payload = build_chat_message_payload(self.base_event, user=None, vote_value=None, current_user=self.viewer)
        self.assertIsNone(payload["user_id"])
    ...
    def test_payload_passes_avatar_url_through(self):
        payload = build_chat_message_payload(self.base_event, user=self.sender, vote_value=None, current_user=self.viewer, avatar_url="/media/avatars/alice.png")
    ...
    def test_payload_avatar_url_forced_to_anonymous_icon_for_anonymous(self):
        event = {**self.base_event, "anonymous": True}
        payload = build_chat_message_payload(event, user=self.sender, vote_value=None, current_user=self.viewer, avatar_url="/media/avatars/alice.png")
    ...
    def test_payload_avatar_url_is_none_when_user_has_no_avatar(self):
        payload = build_chat_message_payload(self.base_event, user=self.sender, vote_value=None, current_user=self.viewer, avatar_url=None)
        self.assertIsNone(payload["avatar_url"])
