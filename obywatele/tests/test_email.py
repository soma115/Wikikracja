...
class NewPersonEmailTest(TransactionTestCase):
    ...
    def test_send_email_to_all_sends_exactly_one_email(self):
        ...
        self.assertEqual(len(mail.outbox), 2, f"Expected 2 emails (one per user), got {len(mail.outbox)}. Double-sending would produce 4 or more.")
    ...
    def test_send_email_to_all_sends_exactly_one_email_on_repeated_calls(self):
        ...
        self.assertEqual(len(mail.outbox), 2, f"Expected 2 emails (one per event), got {len(mail.outbox)}. Each event sends 1 email per user.")

    def test_no_email_when_no_active_users(self):
        self._call_send_email_to_all('Empty subject', 'Empty message')
        self.assertEqual(len(mail.outbox), 0, f"Expected 0 emails with no active users, got {len(mail.outbox)}.")
