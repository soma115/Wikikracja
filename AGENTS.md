...
- **Frontendowy kontrakt WebSocketu (rework czatu, etap F):** socket `/chat/stream/` to współdzielony singleton (`websocket-manager.js::getSharedWebSocket()`) używany przez pełny czat (`wsapi.js`), `notifications.js` i embedded chat (`chat-embedded.js`). Wiadomości i lifecycle przechodzą wyłącznie przez `subscribeMessages(handler)` i `subscribeConnection({onOpen, onClose})` — nigdy bezpośrednie `socket.onmessage`/`onopen`, bo wyciszyłoby to pozostałych konsumentów. Wiadomości bez `__TRACE_ID` są broadcastowane do wszystkich subskrybentów, a wyjątek w jednym handlerze jest izolowany. `subscribeConnection` zarejestrowany po otwarciu socketu dostaje asynchroniczne `onOpen({alreadyOpen: true})` — pierwszy open nie jest gubiony niezależnie od kolejności ładowania modułów; kolejne otwarcia dostają `reconnected: true`. Serwer gubi członkostwo w pokoju przy rozłączeniu, więc każdy konsument sam wykonuje rejoin w `onOpen`; embedded chat buforuje wiadomości (`pendingMessages`) do potwierdzenia joinu (`joinDone`), żeby nie zdublować historii ani nie zgubić wiadomości z okna przejściowego, a `REQUEST_TIMEOUT` joinu jest retry'owany zamiast pokazywania „brak dostępu". `sendJsonAsync` ma timeout 15 s; przy `onClose` oczekujące requesty NIE są odrzucane — `ReconnectingWebSocket` kolejkuje wysyłkę i wypycha ją po ponownym open. Reconnect nie może zmieniać nawigacji (URL, historia, widoczny panel) — pełny czat rejoinuje pokój przez `wsOnReconnect` bez ruszania routingu.
